print("\nINFO: Running four_segments_sinc_hann.py\n")
...
sinc_hamm = sinc * np.hanning(ntap*lblock)
# A longer sinc array, to display the chunks
sinc_long = np.sinc(np.linspace(-ntap/2, ntap/2, ntap*ntap*lblock))
sinc_hamm_long = sinc_long * np.hanning(ntap*ntap*lblock)
# The four quarters (segments) to plot 
seg0 = sinc_hamm_long[:ntap*lblock]
seg1 = sinc_hamm_long[ntap*lblock:2*ntap*lblock]
seg2 = sinc_hamm_long[2*ntap*lblock:3*ntap*lblock]
seg3 = sinc_hamm_long[3*ntap*lblock:]

# Plot the figure
plt.figure(figsize=(7,4.5))
plt.title("".format(ntap,lblock),fontsize=22)
# Plot the sinc-hann window segments
# Segment 0
#plt.plot(seg0, color="green", linewidth=0.8, label="segment 0")
x,y,c = np.arange(0,lblock),sinc_hamm[0:lblock],colors[0]
...
x,y,c = np.arange(lblock,2*lblock),sinc_hamm[lblock:2*lblock],colors[1]
plt.plot(x,y, "--", color=c,linewidth=3)
plt.fill_between(x,y,step="pre",alpha=0.4,color=c)
# Segment 2
x,y,c = np.arange(2*lblock,3*lblock),sinc_hamm[2*lblock:3*lblock],colors[2]
#plt.plot(seg2, color="purple", linewidth=0.8, label="segment 2")
plt.plot(x,y,"--",color=c,linewidth=3)
plt.fill_between(x,y,step="pre",alpha=0.4,color=c)
# Segment 3
#plt.plot(seg3, color="red", linewidth=0.8, label="segment 3")
x,y,c = np.arange(3*lblock,4*lblock),sinc_hamm[3*lblock:4*lblock],colors[3]
plt.plot(x,y,"--",color=c,linewidth=3)
plt.fill_between(x,y,step="pre",alpha=0.4,color=c)
# Plot four big dots
dt = 331 # How far along to plot the dots relative to each resp. segment
times=[2048*i+dt for i in range(4)]
plt.plot(times,sinc_hamm[times],'ko',ms=8)
...
plt.tick_params(
    ...
    labelbottom=False)
plt.xlabel("Timestream Sample Index (Four Frames)", fontsize=16)
plt.ylabel("Sinc-Hann value", fontsize=16)
plt.title("Four segments of Sinc-Hann",fontsize=22)
plt.tight_layout()
# Optionally save the figure
plt.savefig("img/four_segments_sinc_hann.png",dpi=400)
plt.legend()
plt.show(block=True)
